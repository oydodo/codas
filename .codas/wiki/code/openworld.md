...
> An Atlas **code-wiki** page. The prose below is **advisory** — Codas does NOT verify
> its meaning. Only the structural `defines`/`calls`/`contains` claims in the `atlas:claims`
> block are verified (each must resolve to a current node/edge fact). This page doubles as
> agent-readable context for the open-world model.
...
non-resolving `defines` claim is a **warning**, never a hard error.

<!-- The verified surface: each `defines` claim asserts a concept is the top-level symbol at
     node-id <path>::::<name>, which must exist in the current symbol facts. A rename/move of
     any of these without updating this page makes the claim stop resolving -> a code_anchor
     warning (the code->doc drift catch). -->
```atlas:claims
